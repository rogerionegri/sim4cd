...
;IF (Type LE 2)OR(Type EQ 4) THEN Image = FLOAT(Image) ELSE Image = DOUBLE(Image) 
...
;FOR i = 0, N_ELEMENTS(Image[*,0,0])-1 DO BEGIN
;   coefA = 2.0/(MAX(Image[i,*,*]) - MIN(Image[i,*,*]))
;   coefB = 1.0 - (2.0*MAX(Image[i,*,*]))/(MAX(Image[i,*,*]) - MIN(Image[i,*,*]))
;   Image[i,*,*] = coefA*Image[i,*,*] + coefB
;ENDFOR

;FOR i = 0, N_ELEMENTS(Image[*,0,0])-1 DO BEGIN
;   coefA = 1.0/(MAX(Image[i,*,*]) - MIN(Image[i,*,*]))
;   coefB = 1.0 - (MAX(Image[i,*,*]))/(MAX(Image[i,*,*]) - MIN(Image[i,*,*]))
;   Image[i,*,*] = coefA*Image[i,*,*] + coefB
;ENDFOR
